declare global {
    interface Window {
        api: {
            ...
            updateDueDate: (data: { userId: number, dueDate: string }) => { success: boolean, message: string };
            getUsers: () => { success: boolean, message: string, data: [{ id: number, user: string, admission: string, status: string, daysLeft: string, limitValue: string }] };
